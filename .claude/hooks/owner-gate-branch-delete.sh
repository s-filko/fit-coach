...
if printf '%s' "$cmd" | grep -qE 'git[[:space:]]+branch[[:space:]]+(-[a-zA-Z]*[dD]([[:space:]]|$)|--delete([[:space:]]|$))|git[[:space:]]+push[^;|&]*(--delete([[:space:]]|$)|:refs/heads/|[[:space:]]:[[:alnum:]_.][^[:space:]]*[[:space:]]*$)|git[[:space:]]+worktree[[:space:]]+(remove|prune)|git[[:space:]]+update-ref[[:space:]]+(-d|--delete)'; then
