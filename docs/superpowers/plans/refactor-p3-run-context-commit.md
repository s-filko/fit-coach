...
**Goal:** The parent graph state holds only what must survive between runs (`phase`, `activeSessionId`, `messages`, `pendingTransition`); everything about _this_ run (`runId`, `userId`, `user`, `now`, `client`, `trigger`, the metrics collector) travels as run context and is never checkpointed. The topology becomes `prepare → route → <phase> → commit → END`; `commit` persists the run, decides the transition against the domain matrix and guards, and raises one typed transition event whose handlers do the side effects. The route talks to a domain port (`ConversationRunPort.run(input) → { text, phase, runId }`) and no longer imports the graph. Domain imports no LangGraph. Failed runs get a run row.
...
| D-E | `prepare`'s two training short-circuits become `pendingTransition: { toPhase: 'chat', reason: 'session_ended' \| 'session_missing' }` + the catalog reply, `Command({ goto: 'commit' })`; `commit` evaluates them like any transition (training → chat is allowed; `sessionLifecycleHandler` sees an already-completed or missing session and only clears `activeSessionId`; the phase-summary handler runs as today) | Keep the direct `phase: 'chat'` write in `prepare`             | One path for every phase change that is a conversation transition; the registration ↔ chat _sync_ is not a transition and stays a direct write (no event, no summary — as today).                                                                                                                                               |
...
export const TRANSITION_MATRIX: Readonly<
  Record<ConversationPhase, readonly ConversationPhase[]>
>; // today's `allowed` map, verbatim
...
export type TransitionHandler = (
  event: PhaseTransitionCommitted,
) => Promise<void>;
```
...
export const ConversationState = Annotation.Root({
  ...
  pendingTransition: Annotation<TransitionRequest | null>({
    reducer: (_, v) => v,
    default: () => null,
  }),
...
Handler results: `sessionLifecycleHandler` needs to _return_ the new `activeSessionId` (null when it completed/cleared the session). Make `TransitionHandler` return `Promise<Partial<{ activeSessionId: string | null }>>`; `commit` merges the returned partials in order. Keep this in the domain type (Task 2) as `TransitionHandlerResult`.
...
- [x] **Step 1: AC-1334 — NOT RUN (owner quota waiver, 2026-09-17, quoted above).** No
      L1 compare was executed for this plan and no evidence JSON exists for it (an earlier
      draft of this checkbox claimed one — false, corrected at close-out review 2026-09-18).
      AC-1334 is satisfied per the owner decision by: byte-identity of the model-facing
      surface (43/43 snapshots unchanged through Tasks 2–5) + the full unit suite + the
      phase-end dev smoke (Step 2). The phase's L1 evidence is the tool-executor compare:
      `evidence/refactor-p3-tool-executor-l1-compare.json` (365/370, registration −3.2 pp
      documented as sample noise).
- [x] **Step 2: Deploy to dev; P3 phase-end smoke** (2026-09-17, dev @ `bbab7b07`,
      fresh user `Smoke P3` via `/api/bot/user` + `/api/bot/chat`). **Result: registration,
      transitions, run rows, reduced checkpoint state — PASS; training-loop scenarios NOT
      REACHED** — the model never created a `workout_sessions` row because two catalog cards
      (deadlift / romanian deadlift) fail to resolve and the model then submits invalid
      exercise UUIDs, which the tool schema correctly rejects (`Invalid UUID →
sessionTemplates[1].exercises[0].exerciseId` in server logs). Catalog data defect,
      pre-dating P3 (prompts/tool surface byte-identical); filed for follow-up. Evidence:
...
Executed output (psql, dev DB):

```
== last runs (user Smoke P3, thread = user id)
registration||0|ok           -- fresh user greeting, 0 tools
registration||1|ok           -- profile fields saved
registration||1|ok           -- body data saved
registration|plan_creation|1|ok   -- transition RECORDED (trigger=user_message, client=telegram)
plan_creation||11|ok         -- plan saved (11 tool calls)
plan_creation||14|ok         -- plan retry (14 tool calls)
== one row per POST, all outcome=ok (no failed POSTs occurred during the smoke;
   failed-path rows are unit+integration tested: adapter tests, D-F)
== reduced checkpoint state (expect 0) → 0
== conversation_turns for the user → 21
```

Also observed live: the agent node's empty-reply retry (`LLM returned empty response —
retrying once`, recovered); `POST /api/bot/chat` body unchanged `{ data: { content,
timestamp } }`; deploy health check OK (`bbab7b07`).

- [x] **Step 3: AC evidence** (2026-09-18, run in the branch tree):
  - AC-1331: `grep -c "new Map(" apps/server/src/infra/ai/run-metrics.ts` → **0** (the
    only "module-level" hit is a comment stating the P0 maps are gone).
  - AC-1333: `grep -rln "@langchain" apps/server/src/domain/` → **no hits** (exit 1);
    the ESLint rule bites (dedicated override, bite-proven during Task 2).
  - AC-1335: `tests/integration/api/chat.routes.integration.test.ts` —
    `should return the run result text as content`, `should pass userId and text to the
run port`, `should return 500 (no details) when the run throws` — green in the
    591-test suite (now 611 with the close-out review's added tests).
- [x] **Step 4: Docs reconcile** (factual bucket; applied 2026-09-17/18, verified by the
      2026-09-18 four-zone review and completed after it): `docs/ARCHITECTURE.md` (tree: `state.ts`, `nodes/{prepare,route,commit}.ts`, `handlers/`, `conversation-run.adapter.ts`; `domain/conversation/{phases,transitions,events}.ts`; route → port), `docs/CONTRIBUTING_AI.md` (run context, where a run row comes from, how to add a transition handler), `docs/domain/conversation.spec.md` port section (the new port — this is a durable spec: **escalate the exact diff to the owner**, then apply on approval), `docs/BACKLOG.md` ticks: run-metrics binding contract, chat route owns the run-metrics lifecycle, `'unknown'` sentinel, run rows invisible for failed runs (with the D-F ruling), `DrizzleConversationRunService` invents `trigger`/`client`, phase-summary orphan-accumulator test. ADR-0013 amendments to **escalate** (not edit): §4.1 (adapter loads the user; `commit` raises the event; handlers), §3.2 (`metrics` in run context; `promptVersions`/`modelProfile` not in context), §6 (`system_error` and error mapping timing → P5), §4.4 (tools return `ToolReturn`, executor maps to state — from the executor plan).
- [x] **Step 5: Close-out** — done 2026-09-18: four-zone `close-out-review` over the whole P3 phase diff (verdict path: blocked → fixes → independent fix-verification 8/8 PASS → clean; see § Review); checkboxes closed with results, `- Status: done` (all three P3 plans, one phase), `node scripts/state.mjs --write`, commit, push, worktrees removed. `docs/STATE.md`: **P3 complete**; Next → P4 (both plans) and P5 (parallel).
...
Four-zone review of the P3 phase diff (`4b51b0db...bbab7b07`), run 2026-09-18 — the three
P3 plans closed as one phase (owner decision 2026-09-17), so this section is the review
record for all three; the sibling plans reference it. Verdict path: **blocked → fixed →
clean**.

Blocking findings and how each was closed (fix commit `2e2a6bcc` + the docs close-out
commit; every closure verified by an independent fix-verification pass, 8/8 PASS; a full
four-zone re-run was waived per the owner's quota rule of 2026-09-17):

1. R2 — duplicate `AvailabilityInput` type (`phase-spec.ts:44`, zero usages). Closed:
   deleted; single definition in `tool-policy.ts`; `tsc` clean.
2. R3 — eval seeding targeted a different thread than the adapter invokes
   (`run-case.ts` used `${testCase.id}-${runId}`, the adapter uses `thread_id: userId`);
   every eval case would silently start from `registration`. Closed: seeding now uses the
   adapter's thread; verifier confirmed empirically (grep + code path).
3. R3 — the adapter's failed-run path had no tests. Closed:
   `conversation-run.adapter.unit.test.ts` (6 tests: missing user, `core_error` vs
   `llm_unavailable`, best-effort `recordRun`, invoke-config contract).
4. R3 — commit/prepare/handler machinery untested; old persist tests deleted without
   re-homing. Closed: `commit.node.unit.test.ts` (9: collectToolCalls → run row, handler
   order, failing-handler isolation, blocked transition, state clearing, best-effort
   analytics) + `session-lifecycle.handler.unit.test.ts` (6: BR-CONV-018 activation and
   completion).
5. R3+R4 — Task 6 Step 1 ticked with a claim to a nonexistent evidence JSON; Steps 2–3
   claimed pasted outputs that were not pasted. Closed: Step 1 now records the owner's
   L1 waiver (nothing claimed that does not exist); Steps 2–3 paste the actual psql/grep
   outputs from the 2026-09-17 dev smoke.
6. R4 — `docs/domain/conversation.spec.md` Ports section stale (persist.node write site)
   and the new port absent. Closed: owner-approved diff applied 2026-09-18 (owner
   instruction «исправляй»).
7. R4 — `docs/API_SPEC.md`, `FEAT-0003/0008/0009` pre-P3 topology/dead paths. Closed:
   factual reconcile (standing delegation 2026-09-16).
8. R4 — `docs/LLM_CORE_REFACTOR_PLAN.md` AC-1332 said `Command` propagation; §39 said
   persist.node is the temporary run-row home. Closed: aligned with the shipped D-A
   `ToolStateUpdate` mechanism and the commit node, per the owner-approved ADR-0013
   amendments of 2026-09-18 (§3.2/§4.1/§4.2/§4.4/§6 amendment notes in the ADR).

Advisory findings (15) filed in `docs/BACKLOG.md` ("P3 close-out review advisories");
meta findings (4) in `docs/REVIEW_FINDINGS.md`.
