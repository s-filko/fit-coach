...
- [ ] **Step 1: Tests first** — unit: the decorator delegates `run`/`clearContext` through the mutex, same `userId` serialises, different `userId`s do not, a `ThreadBusyError` propagates unchanged and **no run row is written** (D-D: assert the stub `runService.recordRun` was never called for the rejected request). Integration: AC-1351 as described.
- [ ] **Step 2: Implement.**
- [ ] **Step 3: Commit** — `feat(conversation): per-user run mutex on the ConversationRunPort (AC-1351, D-12)`
...
- [ ] **Step 2: Implement.**
- [ ] **Step 3: Commit** — `feat(bot): polling watchdog exits the process on EFATAL or an error burst (AC-1353, BUG-012)`
- [ ] **Step 4: STOP** for orchestrator review.
...
- [ ] **Step 2: Implement.**
- [ ] **Step 3: Commit** — `feat(bot): localized error texts per code, per-chat queue, userId cache (AC-1354)`
- [ ] **Step 4: STOP** for orchestrator review.
...
- [ ] **Step 1: JSDoc and rails** — `withRunMutex`, `createKeyedMutex`, the error classes and `HTTP_STATUS_BY_CODE` all carry JSDoc naming the AC/ADR section they satisfy. Confirm `no-inline-prompts` still passes (no prompt text was added): `npx jest --ci evals/levels/__tests__/no-inline-prompts.unit.test.ts` → pass.
- [ ] **Step 2: Docs reconcile** — the file list above; commit `docs: reconcile ARCHITECTURE, CONTRIBUTING_AI, CLAUDE.md, BUGS with P5 concurrency and error mapping`.
- [ ] **Step 3: Deploy to dev** — merge to `dev`, GitHub Actions deploy, `curl https://fitcoach-dev.filko.dev/health` → 200. Note the **deploy self-update gotcha** (`CLAUDE.md`): workflows run the OLD `deploy.sh`; this plan does not edit `deploy.sh`, so no second manual run is needed — confirm that is still true before deploying.
- [ ] **Step 4: Dev smoke — 3–5 calls only** (owner strategy: no model-backed eval run on this plan). The owner sends 3–5 messages to `@MyFitAiCoachDevBot` across phases. Paste: (a) `docker logs fitcoach-dev-bot --tail 50` showing no polling-error fatal and no watchdog trip; (b) `SELECT outcome, count(*) FROM conversation_runs WHERE created_at > now() - interval '2 hours' GROUP BY 1;` → all `ok`; (c) one deliberate double-send (two messages within a second) and the resulting run rows' `[created_at, created_at + latency_ms]` windows — non-overlapping, or one 409 in the bot log. **No `RUN_LLM_EVALS=1`, no mini-compare, no baseline freeze.**
- [ ] **Step 5: Close-out** — `close-out-review` skill (one review for this plan), tick every checkbox, `- Status: done`, `node scripts/state.mjs --write`, commit, merge. STATE: P5 complete; AC-1351..1354 all closed **now** (none deferred). Branch/worktree cleanup **only on the owner's explicit command** (`CLAUDE.md` rule) — report the branch as ready, do not delete it.
...
| **`requestTimeout` is measured before it is changed (Task 6), and may end up unchanged.** | (a) Raise it to 60 s as the master plan's hypothesis suggests; (b) measure first, change only if the data says so. | **(b)** — the master plan itself labels this a HYPOTHESIS with a verification query. Dev had **no organic traffic** as of 2026-09-19 (recorded in the P4 context-budget plan), so the honest outcome may well be "insufficient data, unchanged" — which is recorded rather than papered over. | If the owner wants it raised regardless: one number in `apps/server/src/app/server.ts:69`. |
