...
- **R4 done** (branch `task/session-investigation-0925-t7`): `formatExerciseSummary(ex, mode)` splits the instruction — `set-triggered` (log_set auto-complete) confirms the reported set first, brief 1–2 line recap last, never announces the next exercise; `explicit` (complete_current_exercise) keeps the old summary + announce wording. Training prompt `v5` (copy of v4, only rule 4 changed) registered as current. AC-SI-4 promoted: tool-text cases → `log-set.tool.unit.test.ts` + new `format-exercise-summary.unit.test.ts`; prompt cases → new `prompts/phases/training/__tests__/training.v5.unit.test.ts`; `exercise-transition-order.repro.test.ts` deleted. Moved snapshots: `registry.unit.test.ts` (`phase.training` → v5), `message-assembly` training×3, `prompt-snapshots` v4 pinned to the frozen `TRAINING_V4` export + new v5×3. Verify: repro `npx jest --testMatch='**/exercise-transition-order.repro.test.ts'` red 5/7 pre-fix (confirm-idx 739 > recap-idx 384; ungated announce sentence) → 7/7 green post-fix; `npm run test:unit` 135/1319 green; through the lock `npm run test:scenarios` 14/371 green (+1 todo).
- **R1 done** (`5fe61768`): `countLlmErrors` (tool-executor.ts) now sums only ToolMessages after the last `HumanMessage` in `state.messages`, plus the current batch; a batch that adds zero new `llm_error`s never appends `tool_error_budget_exhausted`. AC-SI-1a/1b promoted into `tool-executor.unit.test.ts` (AC-1332 block, 2 new its, 27/27 green); AC-SI-1c re-seeded (R2 removed the fractional-RPE error source, so run N now uses two `log_set` calls with an unresolvable `exerciseId`, confirmed red for that exact reason on pre-fix code — catalog fallback delivered for run N+1 despite its own batch having zero errors) and promoted into `tests/integration/scenarios/set-error-recovery.integration.test.ts`; `set-error-recovery.repro.test.ts` deleted. AC-SI-3's case in `tool-error-budget.repro.test.ts` left untouched and still red (R3's). Verify: `npx jest --testMatch='**/tool-executor.unit.test.ts'` exit 0 (28/28, 1 unrelated AC-SI-3 repro suite skipped from this match); `npm run test:unit` exit 0 (133 suites/1310 tests); through the lock `npx jest --testMatch='**/set-error-recovery.integration.test.ts'` exit 0 (2/2); through the lock `npm run test:scenarios` exit 0 (15 suites/373 tests +1 todo).
...
reviewed by its plan). **First pass: blocked** — 4 blocking, 4 advisory. Findings relayed verbatim. **Re-run of R2 and R4 (2026-09-26): clean** — all blocking closed, no new findings.
...
1. **R2** | `apps/server/src/infra/ai/tools/log-set.tool.ts:24-27` and `update-last-set.tool.ts:16-19` |
   ...
   already both import)." — **closed** in `78e9b0a8` (`shared/rpe.ts`)
2. **R2** | `log-set.tool.ts:28-35` and `update-last-set.tool.ts:20-27` | DRY | "`isDatabaseFailure` is
   ...
   `findInErrorCauseChain` in `apps/server/src/shared/pg-error-cause.ts`." — **closed** in `78e9b0a8`
3. **R4** | `docs/STATE.md:11-61` | `SUPERPOWERS_INTEGRATION.md:134`, `:136` | "The AUTO block is out of sync
   with git facts — `node scripts/state.mjs --check` fails right now." — **closed** in `c38e9b01` (check OK)
4. **R4** | `docs/STATE.md:232-238` | `SUPERPOWERS_INTEGRATION.md:134` | "The hand-written 'Handoff
   ...
   word.' … STATE.md was never touched again after `52c0ecb4` despite this major status change." — **closed** in `c38e9b01`

### Advisory

1. **R4** | `docs/BUGS.md:1723,1749,1762,1774,1789` | "BUG-034..038 all still read `Status: Open — red test
   pending (AC-SI-N)`, but … every one has a landed fix with commit SHA and a promoted regression test." — **fixed** in `c38e9b01`
2. **R4** | `apps/server/src/infra/ai/messages/catalog.ts:22` | "Doc comment 'Catalog language driven by
   Telegram's `language_code`; English fallback.' is stale after R3." — **fixed** in `78e9b0a8`
3. **R4** | `apps/server/evals/scenarios/smoke.scenario.ts:501-508` | "The comment above the 'рпе 9-10' step
   still describes the pre-fix bug as live." — **fixed** in `78e9b0a8`
