...
These scenarios require a real LLM and cannot be automated. Run them manually after any changes to the training tools (`apps/server/src/infra/ai/tools/*.tool.ts`), the training PhaseSpec (`graph/phases/training.spec.ts`), or the shared agent node (`graph/nodes/agent.node.ts`).
