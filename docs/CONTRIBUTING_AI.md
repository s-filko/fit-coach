...
2) **One run row per POST** (`conversation_runs`, ADR-0013 §8): the `commit` node records outcome `ok`; the adapter records failed runs (`llm_unavailable` for provider/network errors, `core_error` otherwise), with `error_class` and a truncated `error_message`, before rethrowing. There is no other writer. Run-row semantics: `transition` = `{ toPhase, reason }` from the run's `request_transition` call (null when none); `phaseOut` is set only when the transition **committed** — a blocked or uncommitted request leaves `transition` set with `phaseOut: null`. A run that hops (U5 hand-off, ADR-0013 §8 amendment 2026-09-26) still writes one row: `phaseIn` = the first phase of `phasePath`, `phaseOut` = the last, `transition` = `{ toPhase, reason, path }`, `promptVersions` merged across both phases.
...
### Conversation errors (P5, ADR-0013 §6)
- Throw a **typed** error from the adapter — `LlmUnavailableError` / `ThreadBusyError` /
  `CoreError` (`domain/conversation/errors.ts`) — never let a raw exception or its message
  reach a response body. `chat.routes.ts` maps the typed error's `code` through
  `HTTP_STATUS_BY_CODE` (503 / 409 / 500) and replies `{ error: { code } }` only; the
  original error is kept as `cause` for `req.log.error({ err })`, never for the client
  (INV-LLM-006 is absolute — no HTTP response body may carry `error.message` or a stack).
- The per-userId run mutex (`infra/conversation/with-run-mutex.ts`, D-A/D-12) wraps
  `ConversationRunPort`; a waiter rejected after the wait window throws `ThreadBusyError`
  before the graph is ever entered, so no run row is written for it (D-D).
...
- **Config exception**: `EPISODE_GAP_HOURS` / `EPISODE_MIN_TURNS` / `EPISODE_MIN_TOKENS` are optional env vars with code defaults in `config/index.ts` — the second documented exception to "no defaults in code", after `LLM_PROFILE_*` (tunables, not secrets). `LLM_BUDGET_<PHASE>_<PART>` (PART = `SYSTEM | LONG_TERM | DOMAIN | HISTORY | OUTPUT_RESERVE`, e.g. `LLM_BUDGET_TRAINING_HISTORY=10000`) is the third: it overrides one field of a phase's `PhaseSpec.budget` (`config/llm-budget-overrides.ts`, applied in `conversation.graph.ts`). `LLM_RUN_MUTEX_WAIT_MS` (default 20000, P5, D-A/D-12) is the fourth: how long a waiter for the per-userId run mutex waits before rejecting with `ThreadBusyError` (HTTP 409) — same class as the others, a tunable, not a secret.

### User facts (durable memory, refactor P6)
- **Facts are written only at compaction**: the `compact` step reads the `facts` array of summariser v3's structured output (`summarizer/v3`) and upserts via `IUserFactsService.upsertMany` (`domain/user/ports/user-facts.ports.ts`). Each fact is normalised into a `factKey` and upserted on the unique `(userId, category, factKey)` index — a repeat **increments `confirmations`** and `updatedAt`, never rewrites `fact`. Categories are ADR-0009's eight; a `physical_constraint` may carry a `muscleGroup` (a `MuscleGroup` slug).
- **There is no per-turn fact tool**: `remember_fact` was **dropped by owner decision 2026-09-17** — extraction is compaction-only, and no fact-writing tool may be added.
- **Rendering**: `## User Facts` (block `user_facts.v1`) renders at **block 2** ahead of `## Previous episodes`, budgeted against `longTerm` (ADR-0013 §3.4) — long-term memory precedes episode memory. One line per fact, grouped by category, muscle group appended in parens when set. Zero facts render nothing.
- **Hard validation (AC-1361)**: `save_workout_plan` and `start_training_session` **reject** any exercise whose PRIMARY muscle group matches a `physical_constraint` fact's `muscleGroup` (`getConstraints`), with a `user_error` outcome quoting the fact — the model never gets the last word on a hard constraint. Secondary-muscle overlap is not a conflict.
- **In evals**: the `memory/facts` dataset (`evals/datasets/memory/facts.jsonl` — the first cross-phase dataset directory) carries the already-extracted constraint in `fixture.facts`; the deterministic L1 check `user-facts-block-present` (PROMPT_EVAL_FRAMEWORK §4.2) verifies the assembled input contains the heading and every fact's text.

### Context budget and domain blocks (refactor P4, context-budget plan)
- **Every phase sends the same message shape** (ADR-0013 §3.4): block 1 = the phase prompt (rules/tools/identity, never domain data), block 2 = `## Previous episodes`, block 3 = the phase's **domain context blocks**, then history from the `messages` channel, then the current turn. `assembleContext` (`infra/ai/context/assemble-context.ts`) builds it; `resolveBudget` (`context/budget.ts`) enforces `PhaseSpec.budget` in the INV-LLM-004 order — trim history (whole turns, tool pairs never split, kept tail starts on a human message), step blocks down their declared `depths` (largest first), drop the oldest summary, and as a last resort the D-D floor (block 1 + current only, logged `error`). Block 1 is never cut: `system` over its budget is only logged `warn`.
- **Adding a context block**: create `prompts/blocks/<name>.v1.ts` exporting a `ContextBlock<D>` (`id: '<phase>.<section>'`, `version`, optional `depths` — full depth first, then the reduced ones — and a pure `render(data, ctx, depth) → string | null`; `null` = block absent this run). Declare it in the phase's `contextBlocks` (`graph/phases/<phase>.spec.ts`); the agent node renders from the same `loadContext` data — a block is never a second loader. Prove it with a unit test in `prompts/blocks/__tests__/` and pin its depth variants with a snapshot; the message-assembly snapshots will change and must be regenerated once, with the diff enumerated in the plan. Prompt text belongs under `prompts/` only — `context/**` is scanned for inline prompt literals (BR-LLM-009).
- **Reading the report**: `conversation_runs.budget_report` carries `budget`, per-block `blocks[{ id, tokens, depth }]`, and `cuts` (`'history' | 'block:<id>' | 'summary' | 'floor'`); `SELECT budget_report->'cuts' … WHERE jsonb_array_length(budget_report->'cuts') > 0` shows what trimming did on live traffic.

### Integrate Conversation Context into a Flow
1) Spec:
   - Verify domain rules in `docs/adr/0013-llm-core-target-architecture.md` §3 and the § Memory (episode model) section above. (`docs/features/FEAT-0009-conversation-context.md` and `docs/domain/conversation.spec.md` still describe the pre-P4 context-service model; `conversation.spec.md` is rewritten at master-plan P7, FEAT-0009 is history pending that phase's docs-reconciliation pass — read both as history, not law.)
...
2) Domain:
   ...
   - Persistence goes through `TranscriptPort` / `SummaryPort` (`domain/conversation/ports/`), called by the graph's `commit` / `compact` steps and, for the inbound `human` row only, by the conversation-run adapter before `graph.invoke` so a failing run still keeps what the user wrote — never from a route.
3) Orchestration:
