/**
 * `runWithCleanup` (close-out review advisory 2): the wrapper that decides
 * whether the DB pool gets closed on the way out of `evals/run.ts`, tested
 * entirely with fakes — no live embedding pipeline, no real Postgres pool.
 *
 * The bug this pins: `dbPoolMayBeOpen` used to be set only after `runL3`
 * returned its `'ran'` outcome, so a throw from INSIDE `runL3` (after its own
 * lazy `run-scenario` import had already opened the pool) left
 * `exitAfterCleanup` believing no pool existed — the pool was never closed.
 * A second bug: a rejecting `pool.end()` propagated out of `exitAfterCleanup`,
 * which made the top-level `.catch()` call it a second time — pg rejects a
 * second `pool.end()` with "Called end on pool more than once".
 */
import { runWithCleanup, type CleanupDeps } from '../run';

describe('runWithCleanup (close-out review advisory 2)', () => {
