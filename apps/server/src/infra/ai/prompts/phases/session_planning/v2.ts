...
/**
 * v2 (P4 context-budget plan, Task 2, D-B): v1 minus `client_profile`,
 * `active_plan`, `recent_history`, `recovery_timeline` — they move to the
 * `session_planning.*` domain blocks (block 3, ADR-0013 §3.4). `date` stays
 * in the prompt, still carrying the days-since-last-workout line (D-B: "date
 * stays in the prompt — two lines").
 */
export interface SessionPlanningPromptContextV2 extends DirectiveContext {
...
export const SESSION_PLANNING_V2: PromptModule<SessionPlanningPromptContextV2> = {
  id: 'phase.session_planning',
  version: 'v2',
  directives: DEFAULT_DIRECTIVES_V1,
  render(ctx: SessionPlanningPromptContextV2): Section[] {
    const { user, context, now } = ctx;
