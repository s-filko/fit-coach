...
/**
 * v6 (BUG-030, roadmap R1.3a, training-exercise-history plan D6): identical to v5 except
 * TASK_TEXT rule 1, rewritten around the new `training.exercise_history` / `training.recent_workouts`
 * blocks (which replace `training.previous_session` — it picked one session by exact `session_key`
 * match and missed most of the user's real history), plus one new RULE (11) that forbids denying
 * work the model simply cannot see. Every other TASK/TOOLS/RULES line is unchanged from v5.
 */
// eslint-disable-next-line @typescript-eslint/no-empty-object-type -- v6 adds no fields
export interface TrainingPromptContextV6 extends DirectiveContext {}

const TASK_TEXT = `=== YOUR TASK ===

Guide the client through the workout. At each step:

1. <b>First set of each exercise</b>: Before the first set of each exercise, look it up in EXERCISE HISTORY. Whenever you quote past numbers, say when (the date or "N days ago"); data older than two weeks is never "last time" without its age. No completed record → say so plainly and suggest a conservative start; never borrow kilograms from a different exercise. Check RECENT WORKOUTS for the same muscles: load within the last 48 h → name it and factor it into the recommendation.
...
RULE 11. Past training data exists only in EXERCISE HISTORY and RECENT WORKOUTS. If the user mentions a workout or exercise not shown there, say you do not see it in the records — never tell them they did not do it.
...
export const TRAINING_V6: PromptModule<TrainingPromptContextV6> = {
  id: 'phase.training',
  version: 'v6',
  directives: DIRECTIVES_WITHOUT_IDENTITY_V2,
  render(ctx: TrainingPromptContextV6): Section[] {
    return [
      {
        id: 'intro',
        required: true,
