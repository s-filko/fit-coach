...
/**
 * v5 (session-investigation-0925 R4, BUG-037): identical to v4 except TASK_TEXT
 * rule 4. A transition triggered by the user's own set of the next exercise must be
 * ANSWERED first — confirm that set, reply to what the user said — with the finished
 * exercise's recap as a brief (1-2 lines) aside at the end; "announce the next
 * exercise" survives only in the explicit complete_current_exercise branch (b),
 * where the user actually asked to move on. v4 ordered both transitions
 * summary-first with an unconditional announcement.
 */
// eslint-disable-next-line @typescript-eslint/no-empty-object-type -- v5 adds no fields
export interface TrainingPromptContextV5 extends DirectiveContext {}
...
export const TRAINING_V5: PromptModule<TrainingPromptContextV5> = {
  id: 'phase.training',
  version: 'v5',
  directives: DIRECTIVES_WITHOUT_IDENTITY_V2,
  render(ctx: TrainingPromptContextV5): Section[] {
    return [
      {
        id: 'intro',
        required: true,
