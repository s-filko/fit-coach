...
export function buildManageFactTool(deps: ManageFactToolDeps) {
  ...
  return tool(
    async (input, config) => {
      ...
      if (!input.factId) {
        return llmError('Missing factId: pass the id of the fact (copy it verbatim from list_facts).');
      }

      if (input.operation === 'retract') {
        const retracted = await userFactsService.retractFact(userId, { factId: input.factId }, now);
        if (retracted === null) {
          return llmError(`No fact with id "${input.factId}" for this user. Call list_facts and use the exact id.`);
        }
      ...
      const deleted = await userFactsService.deleteFact(userId, input.factId);
      if (!deleted) {
        return llmError(`No fact with id "${input.factId}" for this user. Call list_facts and use the exact id.`);
      }
      return ok(`Fact deleted entirely: "${input.factId}".`);
    },
    {
      name: 'manage_fact',
